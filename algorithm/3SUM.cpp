    ...
    while(cin >> n){
        ...
        while(cin >> q)
        {
            for(int i = 0; i < n;i++){
                ...
                int qq = q - vec[i];
                while(l < r){
                    if(vec[l] + vec[r] == qq) break;
                    else if(vec[l] + vec[r] < qq) do l++; while(l == i);
                    else if(vec[l] + vec[r] > qq) do r--; while(r == i);
                }

                if(i != l && i != r && l != r && vec[i] + vec[l] + vec[r] == q)
                    cout << "YES" << " " << vec[i] << "+" << vec[l] << "+" << vec[r] <<endl;
            }
        }
