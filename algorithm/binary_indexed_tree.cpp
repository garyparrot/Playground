...
class BIT{
    private:
        int maxi ;
        // 對陣列值 arr[n], 他代表 [n-k+1,n] 之間的合，其中k是可以整除n的最小2倍數, 可以透過 n & -n 快速取得這個
        BIT(int size){ int pow2 = ceil(log2(size)); maxi = 1 << pow2; arr.resize(size+1); }
};
...
int main(int argc, const char* args[]){
    ...
    int l,r;
    while(cin >> l >> r)
        cout << "Query " << l << "," << r << " " << bit.query(l,r) << endl;
}
