...
int main(int argc, const char* args[]){
    ...
    while(cin >> oper){
        if(oper == "FORWARD"){
            while(que.size() > 0 && que.back() > vec[r]) que.pop_back();
            que.push_back(vec[r]);
            r++;
        }else if(oper == "REMOVE"){
            if(que.front() == vec[l]) que.pop_front();
            l++;
        }
        cout << "Current window [" << l << "," << r << "), min value is " << que.front() << endl;
    }
}
