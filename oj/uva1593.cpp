...
int main(int argc, const char* args[]){
    ...
    while(getline(cin, str, '\n')){
        ...
        stringstream ss(str);
        string buf;
        while(ss >> buf){this_line.append(buf);}
        code.append(move(this_line));
    }

    typedef vector<string> vecstr;
    int value = max_element(code.begin(),code.end(), [](const vecstr item1, const vecstr item2) -> bool {
            return item1.size() < item2.size();
    })->size();

#define maxi(a,b) ((a)>(b)?(a):(b))
    vector<int> maxlen(value,0);
    for(int i = 0;i < code.size();i++)
        for(int j = 0;j < code[i].size();j++)
            maxlen[j] = maxi(maxlen[j], (int)code[i][j].length());
    ...
    for(int i = 0;i < code.size();i++){
        for(int j = 0;j < code[i].size();j++)
            if(j == code[i].size() - 1)
                ...
                cout << code[i][j] << string(maxlen[j] + 1 - code[i][j].length(), ' ');
        cout << endl; 
    }
}
